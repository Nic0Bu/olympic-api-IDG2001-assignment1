"""Sport data endpoint with optional query-parameter filtering."""
...
def _aggregate_medals(rows: list) -> dict:
    """Count gold, silver, bronze, no-medal, and total from a list of rows."""
    counts = {
        "gold": 0,
        "silver": 0,
        "bronze": 0,
...
def get_sport(
    ...
    medals: str = QParam(
        None, description="Filter by medal type: gold, silver, or bronze"
    ),
):
    """Return Olympic participation and medal data for a sport.

    The sport_id uses hyphens for spaces (e.g. ski-jumping = Ski Jumping).
    ...
    if medals:
        ...
        if not medal_val:
            raise HTTPException(
                status_code=400,
                detail="medals must be one of: gold, silver, bronze",
            )
